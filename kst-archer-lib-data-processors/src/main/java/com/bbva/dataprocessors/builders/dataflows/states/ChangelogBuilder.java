...
public abstract class ChangelogBuilder<K, V extends SpecificRecordBase> implements StateDataflowBuilder {
    ...
    @Override
    public void build() {
        final Serde<K> keySerde = Serdes.serdeFrom(this.keyClass.getType());
        ...
        builder.addStateStore(entityStore).stream(sourceChangelogTopicName, Consumed.with(keySerde, valueSerde)).transform(() -> {
            return newTransformer(entityStore.name());
        }, new String[]{entityStore.name()}).to(this.snapshotTopicName, Produced.with(keySerde, valueSerde));
