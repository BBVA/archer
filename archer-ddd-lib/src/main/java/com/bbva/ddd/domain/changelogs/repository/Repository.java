...
public final class Repository<K, V extends SpecificRecordBase> {
    ...
    private ChangelogRecordMetadata delete(final String key, final RecordHeaders headers,
                                           final ProducerCallback callback) {
        ...
        try {
            final Future<RecordMetadata> result = producer.remove(record, SpecificRecordBase.class, callback);
        ...
        if (aggregateBaseInstance instanceof AbstractAggregateBase) {
            ((AbstractAggregateBase) aggregateBaseInstance).setDeleteRecordCallback(
                    (method, referenceRecord, callback) ->
                            delete((String) aggregateBaseInstance.getId(), headers(method, referenceRecord), callback));
        }
