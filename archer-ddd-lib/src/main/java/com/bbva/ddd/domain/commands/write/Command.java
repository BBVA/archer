...
public class Command {
    ...
    private Command(final CachedProducer producer, final String topic, final String action, final String key, final SpecificRecordBase value, final RecordHeaders headers, final String uuid) {
    ...
    public CommandRecordMetadata send(final ProducerCallback callback) {
        ...
        if (value != null) {
            result = producer.add(new PRecord<>(topic, key, value, headers), callback);
        } else {
            result = producer.remove(new PRecord<>(topic, key, null, headers), SpecificRecordBase.class, callback);
        }
