...
public final class Repository<K, V extends SpecificRecordBase> {
     ...
     * @param commandMessage reference command record
    ...
    private RecordHeaders headers(final String aggregateMethod, final CRecord record) {
        ...
        recordHeaders.add(ChangelogHeaderType.UUID_KEY, record.recordHeaders().find(CommandHeaderType.ENTITY_UUID_KEY));
        if (record != null && record.key() != null) {
            recordHeaders.add(ChangelogHeaderType.REFERENCE_RECORD_KEY,
                    new ByteArrayValue(record));
        }
